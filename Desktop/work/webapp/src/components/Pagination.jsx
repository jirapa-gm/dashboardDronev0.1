  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5 border-t border-[#3a3a3a] bg-[#1a1a1a]">
      ...
      <div className="flex items-center gap-1">
        {currentPage > 2 && <><PageBtn n={1} {...{ currentPage, onPageChange }} />{currentPage > 3 && <Ellipsis />}</>}
        {pages.map(n => <PageBtn key={n} n={n} {...{ currentPage, onPageChange }} />)}
        {currentPage < totalPages - 1 && <>{currentPage < totalPages - 2 && <Ellipsis />}<PageBtn n={totalPages} {...{ currentPage, onPageChange }} /></>}
      </div>
...
function PageBtn({ n, currentPage, onPageChange }) {
  const active = n === currentPage;
  return (
    <button onClick={() => onPageChange(n)}
      className={[
